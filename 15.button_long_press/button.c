...
static volatile int key_flag[] = {0,0,0,0,0};
...
static void button_press_time(unsigned long dat)
{
	...
	if(pinval)
	{
		if(key_flag[dat] == 0)
			return ;
		else if(key_flag[dat] > 15)
		{
			key_value[dat] =  button_nr_devs;
		}
		else
		{
			key_value[dat] =  1;
	...
	{
		key_flag[dat]++;
		mod_timer(&button_timer[dat],jiffies + (5*HZ/100));
	}
}
static irqreturn_t buttons_interrupt(int irq, void *dev_id)
{
	...
	ret = mod_timer(&button_timer[*des],jiffies + (5*HZ/100));
...
MODULE_DESCRIPTION("15.button long press");
MODULE_LICENSE("GPL");
